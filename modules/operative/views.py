...
from .forms import CreateRequestForm, RequestFilterForm
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if error is None:
                ...
                operative_request.created_by = current_account.username
...
def delete_request_view(request, request_id):
    ...
    current_account = Account.getAccount(request.user)
    try:
        operative_request = OperativeRequest.objects.get(pk=request_id)
    except OperativeRequest.DoesNotExist:
        error = 'No existe una request con el id'
        operative_request = None
    ...
    if error is None:
        if request.method == 'POST':
            operative_request.status = RequestStatus.objects.get(id='9')  # Deleted Status
...
@login_required(login_url="/auth/login/")
def get_request_view(request, request_id):
    request = None
    request_data = {}
    try:
        request:Request = Request.objects.get(pk=request_id)
        request_data['id'] = str(request.id)
        request_data['name'] = request.name
        request_data['fields'] = []
        for request_field in request.request_fields:
            request_field = {
                'field_type': request_field.field_type.id,
                'mandatory': request_field.mandatory,
                'name': request_field.name,
                'options': []
            }
            for request_field_option in request_field.options:
                request_option_data = {
                    'value': request_field_option.value
                }
                request_field['options'].append(request_option_data)
            request_data['fields'].append(request_field)
    except Request.DoesNotExist:
        return 
